...
public class Puzzle {
    ...
    public Puzzle(int h, int w) {
        this.height = h;
        this.width = w;
        this.endingBoard = new Tile[height][width];
        this.startingBoard = new Tile[height][width]; // Inicializamos startingBoard vacío
        this.tilt = new Tilt(startingBoard, glue);
        this.depth = 5;
        ...
        this.endingGlue = new Glue(endingBoard, endingTablero); // Inicializar el objeto Glue con el tablero final
    ...
    private void initializeBoard(Tile[][] board) {
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                board[i][j] = new NormalTile("N", "brown"); // Baldosa normal
            }
        }
    }
    ...
    /**
     * Cambia el tipo de baldosa en una posición específica.
     * @param row Fila de la baldosa
     * @param col Columna de la baldosa
     * @param newType El nuevo tipo de baldosa (por ejemplo: "fixed", "rough", etc.)
     */
    public void setTileType(int row, int col, String newType) {
        // Verificar si la posición es válida
        if (isValidPosition(row, col)) {
            Tile tile = startingBoard[row][col];

            // Verificar si hay una baldosa en la posición
            if (tile != null) {
                tile.setType(newType);
                System.out.println("Baldosa en (" + row + ", " + col + ") cambiada a tipo: " + newType);
            } else {
                System.out.println("No hay ninguna baldosa en la posición (" + row + ", " + col + ").");
            }
        } else {
    ...
    /**
     * Verifica si hay una baldosa en la posición dada y la devuelve.
     * @param row Fila de la baldosa
     * @param col Columna de la baldosa
     * @return La baldosa en la posición o null si no hay baldosa.
     */
    public Tile getTile(int row, int col) {
        if (isValidPosition(row, col)) {
            ...
            if (tile != null) {
                ...
                System.out.println("No hay baldosa en la posición (" + row + ", " + col + ").");
            }
        } else {
            System.out.println("Posición inválida.");
        }
        return null; // Retorna null si no hay baldosa o la posición es inválida
    }
     ...
     * @return true si la posición es válida, false si es inválida
     */
    private boolean isValidPosition(int row, int col) {
        return row >= 0 && row < startingBoard.length && col >= 0 && col < startingBoard[0].length;
    }
}